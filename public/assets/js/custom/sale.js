...
$(".customer-select").on("change", function () {
    ...
    let customer_due = $(this).find(":selected").data("due");
    $('#customer_prev_due').val(customer_due);
...
function showBatchSelectionModal(element, availableStocks, customerType) {
    let html = availableStocks.map((batch, index) => {
        ...
        return `
            <tr class="select-batch"
                ...
                data-route="${element.data("route")}">
                    ...
                    <td class="product_price">${currencyFormat(adjustedPrice)}</td>
            </tr>`;
    }).join("");
...
function populateProducts(products) {
    ...
    $.each(products, function (index, product) {
        ...
        if (product.stocks && product.stocks.length > 1) {
            ...
            product.stocks.forEach((stock) => {
                html += `<div class="d-flex align-items-center justify-content-between w-100 multi-items add-batch-item"
                    ...
                    product.color ? ", Color: " + product.color : ""
                            ...
                            <span class="product-in-stock">In Stock: ${stock.productStock}</span>
                        </div>
            ...
            const singleStock = Array.isArray(product.stocks) && product.stocks.length > 0
                ? product.stocks[0]
                : {};

            html += `<div class="product-option-item single-product ${product.id} add-batch-item"
                ...
                Array.isArray(product.stocks)
                    ? product.stocks.reduce((sum, stock) => sum + stock.productStock, 0)
                    : 0
            }</span>
                            </div>
                            <div class="product-price product_price">${currencyFormat(
...
$("#dropdownList").on("click", ".add-batch-item", function (e) {
    ...
    const text = $(this).find(".product-title").text()?.trim() || $(this).data("product_name");
