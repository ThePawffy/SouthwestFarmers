...
@section('main_content')
    <div class="container-fluid m-h-100">
        <div id="demoAlert" class="custom-alert">
            <span><b class="text-white">Note:</b> This is a demo site. All data will reset automatically every hour.</span>
            ...
            <div class="col-xxl-4 mt-3">
                <div class="card new-card sms-report border-0 p-0 h-100">
                    <div class="card-header">
                        ...
                        <div class="gpt-up-down-arrow position-relative">
                            <select class="form-control overview-year" name="year">
                                @for ($i = date('Y'); $i >= 2022; $i--)
                                    <option @selected($i == date('Y')) value="{{ $i }}">{{ $i }}
                                    </option>
                                @endfor
            ...
            <div class="card">
                <div class="card-bodys">
                    <div class="table-header p-16">
                        ...
                        <div>
                            <a href="{{ route('admin.business.index') }}" class="view-all-btn">{{ __('View All') }}<i
                                    ...
                                    @foreach ($businesses as $business)
                                        <tr class="{{ $loop->iteration % 2 == 0 ? 'even-row' : 'odd-row' }}">
                                            ...
                                            <td class='table-data d-flex align-items-center justify-content-center'>
                                                <div
                                                    class=" @php $subscription = $business->enrolled_plan->plan->subscriptionName ?? null; @endphp
                                                  @switch($subscription)
                                                  @case('Premium')
                                                   btn-premium
                                                   @break
