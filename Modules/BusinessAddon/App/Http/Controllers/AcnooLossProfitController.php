...
use Modules\BusinessAddon\App\Exports\ExportCurrentLossProfit;

class AcnooLossProfitController extends Controller
{
    public function index()
    {
        $profit = Sale::where('business_id', auth()->user()->business_id)->whereYear('saleDate', Carbon::now()->year)->where('lossProfit', '>', 0)->sum('lossProfit');
        $loss = Sale::where('business_id', auth()->user()->business_id)->whereYear('saleDate', Carbon::now()->year)->where('lossProfit', '<=', 0)->selectRaw('SUM(ABS(lossProfit)) as total_loss')->value('total_loss');
        $total_sale = Sale::where('business_id', auth()->user()->business_id)->whereYear('saleDate', Carbon::now()->year)->count();
        $loss_profits = Sale::with('party:id,name')->where('business_id', auth()->user()->business_id)->whereYear('saleDate', Carbon::now()->year)->latest()->paginate(20);

        return view('businessAddon::loss-profits.index', compact('loss_profits', 'profit', 'loss','total_sale'));
    }
     ...
     public function acnooFilter(Request $request)
    {
        ...
        if ($request->ajax()) {
            return response()->json([
                'data' => view('businessAddon::loss-profits.datas', compact('loss_profits'))->render(),
    ...
    public function generatePDF(Request $request)
    {
        ...
        $pdf = Pdf::loadview('businessAddon::loss-profits.pdf', compact('loss_profits'));
