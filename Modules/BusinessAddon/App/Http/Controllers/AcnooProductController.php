...
use Modules\BusinessAddon\App\Exports\ExportProduct;
use Modules\BusinessAddon\App\Exports\ExportExpiredProduct;
...
class AcnooProductController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('businessAddon::products.index', compact('products'));
    }
    ...
    public function create()
    {
        ...
        $product_models = ProductModel::where('business_id', $business_id)->whereStatus(1)->latest()->get();
        ...
        return view('businessAddon::products.create', compact('categories', 'brands', 'units', 'code','vats','product_models','profit_option'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'productPicture' => 'nullable|image|mimes:jpg,png,jpeg,svg|max:2048',
            'productCode' => [
                'nullable',
        ...
        try {
            ...
            $vat_amount = ($basePrice * $vat_rate) / 100;

            $product = Product::create($request->except(['productPicture', 'productPurchasePrice', 'productSalePrice', 'productDealerPrice', 'productWholeSalePrice', 'alert_qty']) + [
            ...
            if (!empty($request->stocks)) {
                foreach ($request->stocks as $stock) {
                    ...
                    $purchasePrice = $request->vat_type === 'inclusive' ? $base_price + ($base_price * $vat_rate / 100) : $base_price;
    ...
    public function show($id)
    {
        ...
        return view('businessAddon::products.create-stock', compact('categories', 'brands', 'units', 'product', 'vats', 'profit_option'));
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('businessAddon::products.edit', compact('categories', 'brands', 'units', 'product', 'vats','product_models', 'profit_option'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // Update product
            $product->update($request->except(['productPicture', 'productPurchasePrice', 'productSalePrice', 'productDealerPrice', 'productWholeSalePrice', 'productStock', 'alert_qty']) + [
                    'business_id' => $business_id,
                    'vat_amount' => $vat_amount,
                    'alert_qty' => $request->alert_qty ?? 0,
    ...
    public function acnooFilter(Request $request)
    {
        ...
        if ($request->ajax()) {
            return response()->json([
                'data' => view('businessAddon::products.datas', compact('products'))->render()
            ]);
        }
    ...
    {
        $expired_products = Product::with('unit:id,unitName', 'brand:id,brandName', 'category:id,categoryName', 'stocks:id,product_id,expire_date,productStock')
    ...
    {
        $expired_products = Product::with('unit:id,unitName', 'brand:id,brandName', 'category:id,categoryName')
            ->where('business_id', auth()->user()->business_id)
            ->whereHas('stocks', function ($query) {
                $query->whereDate('expire_date', '<', today())->where('productStock', '>', 0);
            })
    ...
    // Get customer type & cart batch wise prices
    public function customerWisePrices(Request $request)
    {
        $type = $request->type;
        $stockIds = $request->stocks ?? [];
