...
class AcnooCategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('businessAddon::categories.index', compact('categories'));
    }
    ...
    public function acnooFilter(Request $request)
    {
        ...
        if($request->ajax()){
            return response()->json([
                'data' => view('businessAddon::categories.datas',compact('categories'))->render()
            ]);
        }
    ...
    {
        $request->validate([
            'categoryName' => ['required', 'string', 'max:255', 'unique:categories,categoryName,NULL,id,business_id,' . auth()->user()->business_id],
