...
use Modules\BusinessAddon\App\Exports\ExportLossProfit;
...
class AcnooLossProfitReportController extends Controller
{
    public function index()
    {
        ...
        return view('businessAddon::reports.loss-profits.loss-profit-reports', compact('loss_profits', 'profit', 'loss', 'total_sale_count'));
    }
    ...
    public function acnooFilter(Request $request)
    {
        ...
        if ($request->ajax()) {
            return response()->json([
                'data' => view('businessAddon::reports.loss-profits.datas', compact('loss_profits'))->render(),
    ...
    public function generatePDF(Request $request)
    {
        ...
        $pdf = Pdf::loadview('businessAddon::reports.loss-profits.pdf', compact('loss_profits'));
    ...
    public function lossProfitDetails()
    {
        ...
        $salesQuery = Sale::where('business_id', $businessId)->whereDate('saleDate', $today);
        $purchaseQuery = Purchase::where('business_id', $businessId)->whereDate('purchaseDate', $today);
        $productQuery = Product::where('business_id', $businessId);

        $opening_stock_by_purchase = (clone $productQuery)
            ->whereDate('created_at', '<', $today)
            ->sum(DB::raw('productPurchasePrice * productStock'));

        $closing_stock_by_purchase = (clone $productQuery)
            ->whereDate('created_at', '<=', $today)
            ->sum(DB::raw('productPurchasePrice * productStock'));
        ...
        $opening_stock_by_sale =  (clone $productQuery)
            ->whereDate('created_at', '<', $today)
            ->sum(DB::raw('productSalePrice * productStock'));

        $closing_stock_by_sale = (clone $productQuery)
            ->whereDate('created_at', '<', $today)
            ->sum(DB::raw('productSalePrice * productStock'));
