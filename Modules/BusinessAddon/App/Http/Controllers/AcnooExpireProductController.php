...
class AcnooExpireProductController extends Controller
{
    public function index()
    {
        $expired_products = Product::with('stocks', 'unit:id,unitName', 'brand:id,brandName', 'category:id,categoryName')
            ...
            ->latest()
            ->paginate(20);
            return view('businessAddon::expired-products.index', compact('expired_products'));
    }
    ...
    public function acnooFilter(Request $request)
    {
        ...
        if ($request->ajax()) {
            return response()->json([
                'data' => view('businessAddon::expired-products.datas', compact('expired_products'))->render()
            ]);
        }
