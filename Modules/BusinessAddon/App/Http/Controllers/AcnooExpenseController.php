...
class AcnooExpenseController extends Controller
{
    public function index()
    {
        ...
        return view('businessAddon::expenses.index', compact('expenses', 'expense_categories', 'payment_types'));
    }
    ...
    public function acnooFilter(Request $request)
    {
            ...
            ->when(request('search'), function ($q) use ($request) {
                $q->where(function ($q) use ($request) {
                    $q->where('amount', 'like', '%' . $request->search . '%')
                        ->orWhere('expenseFor', 'like', '%' . $request->search . '%')
        ...
        if ($request->ajax()) {
            return response()->json([
                'data' => view('businessAddon::expenses.datas', compact('expenses'))->render()
            ]);
        }
