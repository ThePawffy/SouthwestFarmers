    ...
    <div class="container-fluid">
            ...
            <form class="sales-header-form">
                <div class="sales-purchese-header">
                    <div class="input-grid">
                        <label class="due-label" for=""> {{ __('Date') }} </label>
                        <input type="datetime-local" name="saleDate" class="form-control" value="{{ \Carbon\Carbon::parse($sale->saleDate)->format('Y-m-d\TH:i') }}">
                    </div>
                    <div class="input-grid">
                        <label for=""> {{ __('Previous Due') }} </label>
                        <input id="customer_prev_due" value="{{ $sale->party->due ?? '' }}" type="number" class="form-control" readonly>
                    </div>
                    <div class="input-grid-calculator">
                        <label for=""> {{ __('Calculator') }} </label>
                    ...
                    <div class="select-customer-lg">
                        <select name="party_id" id="party_id" class="form-select  choices-select customer-select"  aria-label="Select Customer">
                            <option value="">{{ __('Select Customer') }}</option>
                            @foreach ($customers as $customer)
                                <option value="{{ $customer->id }}" data-type="{{ $customer->type }}" data-phone="{{ $customer->phone }}" data-due="{{ $customer->due }}" @selected($sale->party_id == $customer->id)>
        ...
        <div class="grid sales-main-container">
            ...
            <div class="sales-container">
                <form action="{{ route('business.sales.update', $sale->id) }}" method="post" enctype="multipart/form-data" class="sales_purchase_form">
                    @csrf
                    @method('put')
                            ...
                            <div class="col-sm-12 col-md-6 col-lg-6">
                                <div class="amount-info-container">
                                    <div class="row amount-container  align-items-center mb-2">
                                        ...
                                        <input type="number" step="any" name="receive_amount" value="{{ $sale->change_amount + $sale->paidAmount }}" id="receive_amount" min="0" class="form-control" placeholder="0">
                                    </div>
                                    <div class="row amount-container  align-items-center mb-2">
                                        ...
                                        <div class="col-6 w-100 d-flex justify-content-between gap-2">
                                            <div class="d-flex d-flex align-items-center gap-2">
                                                <select name="vat_id" class="form-select vat_select" id='form-ware'>
                                                    ...
                                                    @foreach ($vats as $vat)
                                                        <option value="{{ $vat->id }}" data-rate="{{ $vat->rate }}" @selected($sale->vat_id == $vat->id)>
                                                            {{ $vat->name }} ({{ $vat->rate }}%)
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-6 w-100 d-flex justify-content-between gap-2">
                                            <div class="d-flex d-flex align-items-center gap-2">
                                                <select name="discount_type" class="form-select discount_type" id='form-ware'>
                                                    <option value="flat" @selected($sale->discount_type == 'flat')>
                                                        {{ __('Flat') }} ({{ business_currency()->symbol }})</option>
                                                    <option value="percent" @selected($sale->discount_type == 'percent')>
                                                        {{ __('Percent (%)') }}</option>
                                                </select>
                                            </div>
                                            <input type="number" step="any" name="discountAmount" value="{{ $sale->discount_type == 'percent' ? $sale->discount_percent : $sale->discountAmount }}" id="discount_amount" min="0" class="form-control right-start-input" placeholder="{{ __('0') }}">
                                        </div>
                                    </div>
                                    <div class="row save-amount-container  align-items-center mb-2">
                                        <h6 class="payment-title col-6">{{ __('Shipping Cost') }}</h6>
                                        <div class="col-12 ">
                                            <input type="number" step="any" name="shipping_charge" value="{{ $sale->shipping_charge }}" id="shipping_charge" class="form-control right-start-input" placeholder="{{ __('0') }}">
                                        </div>
                                    </div>
