@foreach ($expired_products as $product)
    ...
    <tr class="{{ $loop->iteration % 2 == 0 ? 'even-row' : 'odd-row' }}">
        <td class="table-data">{{ ($expired_products->currentPage() - 1) * $expired_products->perPage() + $loop->iteration }}</td>

        <td class="table-data">
            <img src="{{ asset($product->productPicture ?? 'assets/images/logo/no-image.jpg') }}" alt="Img" class="table-img">
        </td>
        <td class="table-data">{{ $product->productName }}</td>
        ...
        <td class="table-data {{ $product->stocks_sum_product_stock <= $product->alert_qty ? 'text-danger' : 'text-success' }}">{{ $product->stocks_sum_product_stock }}</td>
        <td class="table-data text-danger">
        ...
        <td class="table-data print-d-none">
