@foreach ($vats as $vat)
    <tr class="{{ $loop->iteration % 2 == 0 ? 'even-row' : 'odd-row' }}">
        ...
        <td class="table-data">{{ $vat->name }}</td>
        <td class="text-dark fw-bold table-data">{{ $vat->rate }}%</td>
        <td class="text-center w-150 table-data">
            <label class="switch">
        ...
        <td>
            <div class="dropdown table-action table-data">
