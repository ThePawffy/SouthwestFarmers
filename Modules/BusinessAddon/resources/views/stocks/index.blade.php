            ...
            <div class="card">
                <div class="card-bodys">
                    <div class="table-header p-16 d-print-none">
                        <h4>{{ request('alert_qty') ? __('Low Stock') : __('Stock List') }}</h4>
                    </div>

                    <div class="table-header justify-content-center border-0 d-none d-block d-print-block text-center">
                        @include('businessAddon::print.header')
                        <h4 class="mt-2">{{ request('alert_qty') ? __('Low Stock') : __('Stock List') }}</h4>
                    </div>

                    <div class="table-top-form p-16">
                        <form action="{{ route('business.stocks.filter', ['alert_qty' => request('alert_qty')]) }}"
                            method="post" class="filter-form" table="#stock-data">
                            @csrf
                            ...
                            <ul>
                                ...
                                <li>
                                    <a href="{{ route('business.stocks.csv') }}">
                                        <img src="{{ asset('assets/images/icons/csv.svg') }}" alt="">
                                    </a>
                                </li>

                                <li>
                                    <a href="{{ route('business.stocks.excel') }}">
                                        <img src="{{ asset('assets/images/icons/excel.svg') }}" alt="">
                                    </a>
                                </li>

                                <li>
                                    <a onclick="window.print()" class="print-window">
                                        <img src="{{ asset('assets/images/icons/printer.svg') }}" alt="">
                                    </a>
                                </li>
                    ...
                    <table class="table bg-striped" id="datatable">
                        <thead>
                            <tr>
                                ...
                                <th class="striped-th text-start">{{ __('Product') }}</th>
                                <th class="striped-th text-start">{{ __('Category') }}</th>
                                <th class="striped-th text-start">{{ __('Cost') }}</th>
                                <th class="striped-th text-start">{{ __('Qty') }}</th>
                                <th class="striped-th text-center">{{ __('Sale') }}</th>
                                <th class="striped-th text-end">{{ __('Stock Value') }}</th>
                            </tr>
                        </thead>
                        <tbody id="stock-data">
                            @include('businessAddon::stocks.datas')
                        </tbody>
                    </table>
                </div>
                <div class="mt-3">
...
@push('modal')
    @include('businessAddon::stocks.stock-modal')
@endpush
